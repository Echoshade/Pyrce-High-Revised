...
Beatrice_Buttons
	...
	Hide
		...
		Click()
			if(usr.playing==0&&usr.beatrice==0)return
			else/*if(usr.beatrice==1) This was my proposed fix, maybe not needed.*///if(gamemode=="Witch")
				usr.Toggle_visibility()
	Butterflies
		icon_state="fire"
...
var/shs1[]=list(new/Buttons/Shoot) Temporarily deprecated because I don't know how to make the 7HS work properly yet.
var/shs2[]=list(new/Buttons/Seven_Transform)*/
	...
	if(usr.client)
		...
		if(usr.playing==1)
			...
			if(usr.currentrole=="The Doppelganger"&&usr.playing==1)
				stat(dg)
			/*if(usr.equipname=="Seventh Holy Scripture") //Temporarily deprecated, sorry. Still don't know how to make this work.
				...
				if(Q.transformed==0)
					...
					stat(shs2)*/
			if((usr.currentrole=="The Witch" || usr.beatrice==1)&&usr.playing==1)
				stat(hide)
				/*//var/obj/weapons/Q=usr.equipped
				var/Q=usr.butterflies Can this var declaration even work?
				if(Q.butterflies==1)
					stat(butterflies)
				if(Q.butterflies==0)
					stat(butterflies2)*/
...
var/list/BannedPeople=list()//"CleaverGirl") Tentatively deprecated. How bad could they be?
proc
	worldsave()
