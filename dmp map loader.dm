...
mob
	verb
		MusicAdjust()
			set category=null
			set hidden=1
			var/P=winget(usr,"musicer","value")
			usr.mastervolume=round(text2num(P))
			if(playi)
				var/sound/S = usr.playi
				//P=winget(usr,"musicer","value")
				S.volume=round(text2num(P))
