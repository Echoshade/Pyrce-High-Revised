...
var/halllights=1
proc/ToggleHallLightsOn()
	//var/turf/T = loc
	//var/area/demoarea/outside/O
	for(var/turf/School_Floors/T in world)
	//if(!T) return	// for some reason, the mob is not on a turf.

		if(istype(T.loc,/area/outside))	// if the turf is in an outside area
			//var/area/I=T.loc
			//if(I.trueoutside==1)
			T.sd_SetLuminosity(4)
proc/ToggleHallLightsOff()
	//var/turf/T = loc
	//var/area/demoarea/outside/O
	for(var/turf/School_Floors/T in world)
	//if(!T) return	// for some reason, the mob is not on a turf.
	...
	proc
		...
		sd_StripLum(list/V = view(luminosity,src), center = src)
			...
			for(var/turf/T in V)
				...
				//	update the turf's area
				T.sd_LumUpdate()
		sd_ApplyLum2(list/V = view(luminosity,src), center = src)
			if(src.luminosity>sd_top_luminosity)
				sd_top_luminosity = src.luminosity
			// loop through all the turfs in V
			for(var/turf/T in V)
		...
		sd_StripLum2(list/V = view(luminosity,src), center = src)
			// loop through all the turfs in V
			for(var/turf/T in V)
				/*	increase the turf's brightness depending on the
					brightness and distance of the lightsource */
		...
		sd_SetLuminosity(new_luminosity as num)
			...
			if(luminosity>0)
				sd_ApplyLum()
		sd_SetLuminosity2(new_luminosity as num)
			/*	This proc should be called everytime you want to change the
				luminosity of an atom instead of setting it directly.
		...
		sd_LumUpdate()
			...
			if(Loc.tag!=ltag)	//skip if already in this area
				...
				if(!A)
					...
					A.tag = ltag
					//i//f(A.trueoutside==1)return
					if(A.sd_outside)
						if(!(A in sd_outside_areas))
							sd_outside_areas += A
						//if(A.trueoutside==1)goto endd
