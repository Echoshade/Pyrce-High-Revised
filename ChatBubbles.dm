...
mob
	...
	proc
		BubbleTalk(duration = 200)
			src.BubbleCustom2(ICON_NAME,"talk",duration)
		...
		BubbleCustom2(icon as icon,icon_state as text,duration = 10)
			del(src.s_bubble)
			src.s_bubble = new
			spawn() src.s_bubble.DisplayChat(src,icon,icon_state,duration)
obj/s_bubble/proc/Display(mob/M,icon,icon_state,duration)
	...
	sleep(2)
	del(src)
obj/s_bubble/proc/DisplayChat(mob/M,icon,icon_state,duration)
	M.istyping=1
	src.layer = 6
	src.pixel_y=8
	src.pixel_x=6
	src.name="Typing..."
	if(M.isGhost==1)src.invisibility=50
	var/turf/T = locate(M.x,M.y,M.z)
	if(T)
		src.loc = T
	else
		src.loc = M.loc
